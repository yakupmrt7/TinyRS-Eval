...
#SBATCH --output=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output/eval-cls-cot-%A_%a.out
#SBATCH --error=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output/eval-cls-cot-%A_%a.err
...
# Stage-2 CoT model: Qwen3.5-0.8B -> +SFT LoRA -> cls-merged -> +CoT LoRA -> cot-cls-merged
MODEL=/arf/scratch/aalatan/Re-CoT/Qwen-VL-Series-Finetune/output/qwen35-cot-cls-merged
DATA_ROOT=/arf/scratch/aalatan/Re-CoT/datasets_eval
OUTPUT=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output

# NOTE: do NOT pass --template_config. The CoT models were trained with no system turn at
# all (Qwen3.5 gets no default system message), so the thinking template's meta_instruction
# is an unseen ~90-token prefix. Measured A/B on the VQA CoT model: with it, 0/6 outputs
# were well-formed (accuracy 0.0); without it, 6/6. See diagnosis_vqa.md.
...
# max_new_tokens 1024 (not the 128 used for the SFT models): CLS CoT traces are ~400 tokens
# of reasoning BEFORE the <answer> tag, so at 128 the answer never appears and every sample
# scores 0.
$PY "$SCRIPT" \
    --model_path "$MODEL" \
    --data_root "$DATA_ROOT" \
    --anns_json "$ANNS" \
    --output_dir "$OUTPUT" \
    --model_name qwen3.5-0.8b-cot-cls-merged \
