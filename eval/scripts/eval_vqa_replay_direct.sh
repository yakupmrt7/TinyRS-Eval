#!/usr/bin/env bash
#SBATCH --job-name=eval-vqa-replay-direct
...
#SBATCH --output=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output/eval-vqa-replay-direct-%A_%a.out
#SBATCH --error=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output/eval-vqa-replay-direct-%A_%a.err

# DIRECT (non-thinking) mode eval of the stage-2 REPLAY VQA model (SFT-merged + replay LoRA). Prompt uses the default closed
# <think>\n\n</think> scaffold, so the model emits a plain answer -- this is the number to
# compare against the stage-1 SFT (mean 81.2%). Success = at/above SFT.
...
# no --enable_thinking (closed think scaffold), max_new_tokens 128 (plain answer)
$PY "$SCRIPT" \
    --model_path "$MODEL" \
    --data_root "$DATA_ROOT" \
    --anns_json "$ANNS" \
    --output_dir "$OUTPUT" \
    --model_name qwen3.5-0.8b-replay-vqa-direct \
