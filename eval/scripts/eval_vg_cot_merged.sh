...
#SBATCH --output=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output/eval-vg-cot-%j.out
#SBATCH --error=/arf/scratch/aalatan/Re-CoT/TinyRS/eval/output/eval-vg-cot-%j.err
...
# Stage-2 CoT model: Qwen3.5-0.8B -> +SFT LoRA -> vg-merged -> +CoT LoRA -> cot-vg-merged
MODEL=/arf/scratch/aalatan/Re-CoT/Qwen-VL-Series-Finetune/output/qwen35-cot-vg-merged
...
# bbox_normalize_bound 800: the CoT VG data was rescaled 0-1000 -> 0-800 (both the <answer>
# box and the coordinates quoted in the reasoning prose), matching the stage-1 VG SFT model.
# DIOR images are 800x800 with absolute-pixel GT, so 0-800 preds map 1:1 to pixels.
# max_new_tokens 1024: VG CoT traces are ~280 tokens of reasoning before the <answer> tag.
$PY "$SCRIPT" \
    --model_path "$MODEL" \
    --data_root "$DATA_ROOT" \
    --anns_json VG_DOIR_RSVG_test.json \
    --output_dir "$OUTPUT" \
    --model_name qwen3.5-0.8b-cot-vg-merged \
